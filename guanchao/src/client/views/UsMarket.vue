...
usePoll(() => {
  load()
  /* 指数刷新更贵，按分钟窗口节流：只在每分钟前 15 秒内顺带刷新 */
      ...
      <table class="tbl">
        ...
        <tbody>
          ...
          <tr v-for="(s, i) in sorted" :key="s.code" @click="openDetail(s)">
            ...
            <td data-label="涨跌幅" :class="s.price == null ? '' : cl(s.pct)">{{ s.price == null ? '—' : pc(s.pct) }}</td>
            <td data-label="涨跌额" :class="s.price == null ? '' : cl(s.change)">{{ s.price == null ? '—' : sg(s.change) + fx(s.change) }}</td>
