...
const hkIndices = ref<MarketIndex[]>([])
const label = ref('')
...
/* 顶部恒生/恒生科技/国企/红筹 四大指数（与美股「三大指数」KPI 卡片一致） */
async function loadHKIndices() {
  try {
    hkIndices.value = (await api.hkIndex('index')).list || []
  } catch {
    /* 指数失败不影响板块表格 */
  }
}
...
<template>
  <div>
    ...
    <!-- 四大指数 -->
    <div v-if="hkIndices.length" class="kpi-row">
      <div v-for="x in hkIndices" :key="x.secid" class="kpi">
