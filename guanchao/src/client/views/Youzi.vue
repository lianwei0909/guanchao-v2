...
import YouziPortraitModal from '@/components/YouziPortraitModal.vue'
            ...
            <tr v-for="(d, i) in filteredDetail" :key="`${d.code}-${d.dept}-${i}`">
              <td data-label="代码/名称">
                <span
                  ...
                  @click.stop="detailStock = { code: d.code, name: d.name, secid: secidOf(d.code) }"
                >{{ d.name }}</span>
